...
def visualize_clusters(clustered_rfm_df):
    ...
    sns.scatterplot(x='MonetaryValue', y='Frequency', hue='Cluster', data=clustered_rfm_df, palette='viridis', legend='full')
...
def visualize_clusters_customers(clustered_rfm_df):
    ...
    for index, row in clustered_rfm_df.iterrows():
        plt.scatter(row['MonetaryValue'], row['Frequency'], color=plt.cm.viridis(row['Cluster'] / max(clustered_rfm_df['Cluster'])), alpha=0.5)
    plt.title('RFM Clusters')
    plt.xlabel('MonetaryValue')
    plt.ylabel('Frequency')
    plt.show()
...
def main():
    ...
    active_customers_df = get_active_customers(df_2023, 12)
